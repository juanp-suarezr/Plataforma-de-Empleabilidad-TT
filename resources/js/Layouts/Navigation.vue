<template>
    ...
    <div :class="$page.props.showingMobileMenu
        ...
        " :href="route('buscarOfertas')" :active="route().current('buscarOfertas')">
                <MagnifyingGlassIcon class="h-6 w-6 text-white" />
                <span class="mx-3">Busqueda de ofertas </span>
            </nav-link>
            <!-- Publicacion trabajo (campista) -->
            <nav-link v-if="$page.props.user.permissions.includes('perfilProfesional-list')
        " :href="route('about')" :active="route().current('about')">
            ...
            <SeparadorMenu v-if="$page.props.user.permissions.includes('usuarios-listar')">Analisis</SeparadorMenu>
            <!--  Análisis de datos -->
